...
import base64
from country_map import alpha2_to_alpha3, alpha2_to_name
from utils import truncate_name, load_genre_cache, save_genre_cache, get_week_range, format_period, format_month, week_to_datetime

GENRE_CACHE_FILE = "artist_genres_cache.json"
...
def get_top_per_period_plot(song_df, entity, metric, granularity):
    ...
    if entity == "Song":
        ...
        top_per_period["Label"] = top_per_period.apply(
            lambda row: f"{row['Period']} – {row['Artist']}",
            axis=1
        )
    elif entity == "Album":
        top_per_period["Label"] = top_per_period.apply(
...
def get_top_listening_combos(song_df, entries, metric, granularity, entity_type):
    ...
    if entity_type == "Song":
        ...
        top_entries["Label"] = top_entries.apply(
            lambda row: f"{row['FormattedPeriod']} - {row['Artist']}",
            axis=1
        )
    elif entity_type == "Album":
        top_entries["Label"] = top_entries.apply(
...
def get_top_genres_plot_data(song_df):
    ...
    cache = load_genre_cache(GENRE_CACHE_FILE)
    ...
    if missing_artist_names:
        ...
        save_genre_cache(cache, GENRE_CACHE_FILE)
...
with tab1:
  ...
  st.subheader("How to get your file?")
